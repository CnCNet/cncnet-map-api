...
from kirovy.services.cnc_gen_2_services import CncGen2MapParser, CncGen2MapSections, ParseErrorMsg
...
def test_map_parser_service__validate(file_binary):
    ...
    assert ParseErrorMsg.NO_BINARY in str(exc_info.value)
...
def test_map_parser_service__fails_parse(file_map_cant_parse):
    ...
    assert ParseErrorMsg.CORRUPT_MAP in str(exc_info.value)
...
def test_map_parser_service__fails_missing_sections(file_map_missing_sections):
    ...
    assert ParseErrorMsg.MISSING_INI in str(exc_info.value)
...
def test_map_service_can_extract_preview(file_map_valid, file_map_snow, file_map_desert, tmp_media_root):
    ...
    for map_file, width, height in maps:
        ...
        assert map_service.ini.get(CncGen2MapSections.PREVIEW, "Size") == f"0,0,{width},{height}"
